...
{
    class SqlEmployeeRepository : IEmployeeStrategy
    {
        ...
        public List<Employee> GetAll()
        {
            ...
            {
                var command = new SqlCommand("SELECT Users.*, Employees.OrdersProcessed, Employees.DaysWorked, Employees.DateHired, " +
                                             "Employees.DateFired, Employees.DateLastSalaryPaid, Employees.BankAccountNumber, " +
        ...
        public Employee Get(int id)
        {
            ...
            using (var connection = new SqlConnection(_connectionString))
            {
                var command = new SqlCommand("SELECT Users.*, Employees.OrdersProcessed, Employees.DaysWorked, " +
                                             "Employees.DateHired, Employees.BankAccountNumber, Employees.TotalSalaryPaid, " +
                                             "Employees.IsWorkingNow FROM Employees " +
        ...
        public void Add(Employee employee)
        {
            using (var connection = new SqlConnection(_connectionString))
            {
                ...
                command.CommandText = "INSERT INTO Employees (UserId, OrdersProcessed, DaysWorked, DateHired, BankAccountNumber, TotalSalaryPaid, IsWorkingNow) " +
                                      "VALUES (@UserId, @OrdersProcessed, @DaysWorked, @DateHired, @BankAccountNumber, @TotalSalaryPaid, @IsWorkingNow);";
        ...
        public void Update(Employee employee)
        {
            using (var connection = new SqlConnection(_connectionString))
            {
                var command = new SqlCommand("UPDATE Users SET Username = @Username, Salt = @Salt, HashedPassword = @HashedPassword, " +
                                             ...
                                             "DateHired = @DateHired, BankAccountNumber = @BankAccountNumber, TotalSalaryPaid = @TotalSalaryPaid, " +
        ...
        public Employee MapSqlReaderToObject(SqlDataReader reader)
        {
            return new Employee
            {
                ...
                TotalSalaryPayed        = reader.GetDouble(reader.GetOrdinal("TotalSalaryPayed")),
